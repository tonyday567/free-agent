...
-- | Kimi-backed agent runner for the free-agent bus.
--
-- Watches ROOT/log.jsonl for posts addressed to NAME(s), forwards each body to
-- kimi with the given prompt prepended as system context, and writes the
-- cleaned reply lines back through the scribe with proper thread edges.
--
-- Usage:
--   free-agent-bus-kimi ROOT NAME [NAME...] PROMPT.md [SESSION-FILE]
...
import Free.Agent.Host (Host (..), kimiHost)
...
usage :: IO ()
usage = do
  TIO.putStrLn "Usage: free-agent-bus-kimi ROOT NAME [NAME...] PROMPT.md [SESSION-FILE]"
  ...
  TIO.putStrLn "  SESSION-FILE    optional kimi session file (default: ROOT/.sessions/NAME.sid)"
  ...
  TIO.putStrLn "  --model MODEL   model passed to kimi -m (default: CLI default)"
  TIO.putStrLn "  --provider P    provider passed to kimi --provider (default: CLI default)"

-- | Drop kimi CLI noise lines that can precede the actual reply text,
  ...
  case parseArgs args of
    ...
    Just (root, names, promptFile, sessionFile, mQuiesce, mModel, mProvider) -> do
      ...
      let agentName = case names of (n : _) -> n; [] -> "agent"
          -- kimiHost takes no system prompt, so the prompt text is
          -- prepended to every body inside the query, hermes-style.
          host0 = kimiHost agentName mModel mProvider sessionFile
          host =
            host0
              { hostRun =
          ...
          seat = hostSeat host
      scribe <- findScribe
      TIO.putStrLn $ "🟢 bus kimi agent starting: " <> T.intercalate "," names
